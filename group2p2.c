...
//    memset(&(their_addr.sin_zero), '\0', 8);
    ...
    while((newfd = accept(sockfd2, (struct sockaddr *)&their_addr, &sin_size)) != -1)//bypass-server connected
    {
        //char buf[1000];
        int len = recv(sockfd, &their_addr2, sin_size, 0);
        //buf[len] = '\0';
        //printf("%s", buf);
        //buf[0] = '\0';
        //char num[100];
        ...
        sleep(5);
        close(newfd);
        if((connect(sockfd, (struct sockaddr *)&their_addr2, sin_size)) > 0)
        {
